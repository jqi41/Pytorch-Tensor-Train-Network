...
def train_vae(model, optimizer, n_epochs):
    ...
    for epoch in range(1, n_epochs+1):
        ...
        for i, (x_mb, y_mb) in enumerate(train_loader):
                ...
                loss.backward()
                optimizer.step()
        test_vae(model, optimizer)
        print()
    
    torch.save(model.state_dict(),"model_weights/vae_"+ time.strftime("%m%d-%H%M")+ ".pt")
    
    return model, epoch

def test_vae(model, optimizer):
...
model, _ = train_vae(modelN, optimizerN,5)
